...
uint8_t Serial_RxPacket[100];//注意参数量值
...
void USART1_IRQHandler(void)//mds中找，在中断函数里面接收数据,接收数据包然后存在RXPACKET里
{
	static uint8_t RxState=0;//接收前标志一下当前状态
	static uint8_t pRxPacket=0;//指示接收到哪一位数据
	
	if(USART_GetITStatus(USART1,USART_IT_RXNE)==SET)//先判断中断标志位
	{
		uint8_t RxData=USART_ReceiveData(USART1);//获取RxData
		if(RxState==0)//当前状态等待包头
		{
		...
		else if(RxState==1)//等待接收数据，加else防止两个if同时成立
		{
			if(RxData=='\r')//不确定多少个字符所以每次都要判断是不是末位
			{
				RxState=2;
			}
			else
			{
				Serial_RxPacket[pRxPacket]=RxData;//[1]表示[]里一共有几个数
				pRxPacket++;
			}
			}
		else if(RxState==2)//等待包尾
		{
			...
			{
				RxState=0;
				Serial_RxPacket[pRxPacket]='\0';//字符串最后要'\n'(回车键换行)结束
				Serial_RxFlag=1;
			}
		}
