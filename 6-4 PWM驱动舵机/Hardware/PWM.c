...
void PWM_Init(void)
{
	...
	TIM_TimeBaseInitStructure.TIM_Period=20000-1;//周期ARR自动重装器的值,预分频器和计数器都有1个数的偏差，数到10清零
	TIM_TimeBaseInitStructure.TIM_Prescaler=72-1;//PSC预分频器的值，1s=1hz，套用公式设置的具体数值可变,0不分频
	...
	TIM_OCInitStructure.TIM_Pulse=0;//TIM_Pulse脉冲，用于设置CCR寄存器的值，舵机要求0.0-2.5Ms
	TIM_OC2Init(TIM2,&TIM_OCInitStructure);//结构体成员没有配置完整会导致有些不能输出等问题,用通道2输出	
	
	TIM_Cmd(TIM2,ENABLE);//最后启动定时器 
}
void PWM_SetCompare2(uint16_t Compare)
{
	TIM_SetCompare2(TIM2,Compare);//在运行过程中更改CCR实现呼吸灯效果
}
