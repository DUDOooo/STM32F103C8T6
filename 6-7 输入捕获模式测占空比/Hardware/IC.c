...
void IC_Init(void)
{
	...
	TIM_TimeBaseInitStructure.TIM_Prescaler=72-1;//72M/72=1MHZ,PSC预分频器的值，1s=1hz，套用公式设置的具体数值可变,0不分频,调大PSC就可以得到更小值的下限频率
	...
	TIM_PWMIConfig(TIM3,&TIM_ICInitStructure);//直接可以对上面取反，tim.c中有说明，只支持通道1和2
...
{
	return 100000/(TIM_GetCapture1(TIM3)+1);//hz,TIM_GetCapture1(TIM3)捕获到的CCR的值
}

uint32_t IC_GetDuty(void)
{
	return (TIM_GetCapture2(TIM3)+1)*100/(TIM_GetCapture1(TIM3)+1);//hz,TIM_GetCapture1(TIM3)捕获到的CCR的值,CCR2/CCR1=占空比，计算结果0~1
}
